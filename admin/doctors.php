...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_doctor'])) {
    ...
    $password = $_POST['password']; // Admin-provided password
    ...
    $stmt = $conn->prepare("INSERT INTO doctors (name, specialization, contact_number, image_path, available, username, password_hash, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
    $stmt->bind_param("sssssss", $name, $specialization, $contact, $image_path, $available, $username, $password_hash);
    $stmt->execute();
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_doctor'])) {
    ...
    $stmt = $conn->prepare("UPDATE doctors SET name=?, specialization=?, contact_number=?, available=?, image_path=?, username=? $password_sql WHERE id=?");
...
table { width: 100%; border-collapse: collapse; background: white; box-shadow: 0 0 10px rgba(0,0,0,0.05); border-radius: 8px; overflow: hidden; }
...
h1, h2 { margin-top: 0; }
form { margin-top: 30px; background: white; padding: 25px; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.05); }
.form-group { margin-bottom: 15px; }
label { display: block; margin-bottom: 5px; font-weight: 600; }
input[type="text"], select, input[type="file"], input[type="password"] { width: 100%; padding: 10px; border-radius: 5px; border: 1px solid #ccc; outline: none; transition: 0.2s; }
input[type="text"]:focus, select:focus, input[type="password"]:focus { border-color: #007bff; }
input[type="submit"] { background-color: #28a745; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-weight: 600; }
.delete-link, .edit-link { text-decoration: none; margin-right: 10px; font-weight: 600; }
  ...
  <div class="main-content">
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td><?= $row['id'] ?></td>
          <td><?= $row['image_path'] ? '<img src="'.htmlspecialchars($row['image_path']).'" alt="Doctor">' : 'N/A' ?></td>
